...
Modified for better GitHub sync compatibility
"""
...
# MODIFIED: Force fresh file reads
def load_realtime_prices():
   """Load real-time prices - NO CACHING"""
   price_file = Path("realtime_prices.json")
   if price_file.exists():
       try:
           # Force re-read by using file modification time
...
# Main Dashboard
def main():
   # Force clear any Streamlit cache periodically
   if st.session_state.counter % 20 == 0:
       st.cache_data.clear()
   ...
   with placeholder.container():
       ...
       # MODIFIED: More aggressive connection checking
       is_connected = False
       connection_source = ""
       
       # Check status.json timestamp - reduced timeout to 45 seconds
       if status and 'timestamp' in status:
           try:
               ...
               if time_diff < 45:  # Reduced from 120 to 45
                   is_connected = True
                   connection_source = "status"
           except:
               pass
       
       # Also check if we have recent signals (within last 2 minutes)
       ...
       # Add refresh notice for Streamlit Cloud users
       if not is_connected and market_status == "MARKET OPEN":
           st.markdown("""
           <div class="refresh-notice">
               ⚠️ Dashboard shows offline? Refresh the page (F5) to get latest data from GitHub
           </div>
           """, unsafe_allow_html=True)
       ...
       <div style="text-align: center; color: var(--text-tertiary); font-size: 11px; 
           ...
           Auto-refresh: 2 seconds{price_status} • 
           {'🟢 Connected' if is_connected else '🔴 Disconnected'} {connection_info} • 
           🔔 Sound notifications enabled • 
           📱 Refresh page (F5) for latest data
       </div>
       """, unsafe_allow_html=True)
   
   # Auto-refresh - MODIFIED: Faster refresh for better responsiveness
